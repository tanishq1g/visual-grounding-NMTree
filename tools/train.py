...
def main(opt):
    ...
    ## changed file to support pytorch 0.4
    opt.data_file = 'data_dep_dict'
    data_pth = os.path.join(opt.feats_path, opt.dataset_split_by, opt.data_file + '.p')
    visual_feats_dir = os.path.join(opt.feats_path, opt.dataset_split_by, opt.visual_feat_file)
    # print('data_json, data_pth, visual_feats_dir', data_json, data_pth, visual_feats_dir)
    # data_json, data_pth, visual_feats_dir data/feats/refcocog_umd/data_dep.json data/feats/refcocog_umd/data_dep_dict.p data/feats/refcocog_umd/matt_res_gt_feats.pth
    ...
    while True:
        ...
        if wrapped and epoch % opt.save_checkpoint_every == 0:
            ...
            with open(os.path.join(checkpoint_dir, 'infos.json'), 'w') as f:
                json.dump(infos, f, sort_keys = False, indent = 4)
