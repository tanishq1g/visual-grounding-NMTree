...
def detect(cfgfile, weightfile, imgfile):
    ...
    start = time.time()
    boxes, layer74 = do_detect(m, sized, 0.5, 0.4, use_cuda)
    print("boxes ", boxes, np.shape(boxes), do_detect, m.width, m.height)
    correct_yolo_boxes(boxes, img.width, img.height, m.width, m.height)

    finish = time.time()
    print("after correct yolo ", boxes, img.width, img.height)
